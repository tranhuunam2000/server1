...
RegisterNUICallback('escape', function(data, cb)
	if hasIdentity then
		...
		TriggerEvent('chat:addMessage', { args = { '^1[IDENTITY]', '^1Bạn phải tạo nhân vật đầu tiên của mình để chơi' } })
	end
end)
		...
		elseif theData == "height" then
			...
			if height then
				if height > 200 or height < 140 then
					reason = "Chiều cao người chơi không được chấp nhận"
					break
				end
			else
				reason = "Chiều cao người chơi không được chấp nhận"
...
function verifyName(name)
	...
	if nameLength > 25 or nameLength < 2 then
		return 'Tên người chơi của bạn quá ngắn hoặc quá dài.'
	end
	...
	if count ~= nameLength then
		return 'Tên người chơi của bạn chứa các ký tự đặc biệt không được phép trên máy chủ này.'
	end
	...
	if spacesInName > 2 then
		return 'Tên của bạn chứa nhiều hơn hai khoảng trắng'
	end
	
	if spacesWithUpper ~= spacesInName then
		return 'Tên của bạn phải bắt đầu bằng chữ in hoa.'
	end
