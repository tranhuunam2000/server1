	...
	MySQL.Async.fetchAll("SELECT * FROM owned_vehicles where plate=@plate and owner=@identifier",{['@plate'] = vehplate, ['@identifier'] = identifier}, function(result)  
		if result[1] ~= nil then
			...
			if originalvehprops.model == vehiclemodel then
				...
				DropPlayer(_source, "Bạn đã bị kích khỏi server do cố gắng gian lận")
				print("[esx_eden_garage] player "..identifier..' đã cố gắng sinh ra một chiếc xe:'..vehiclemodel..". xe nguyên bản của anh ấy: "..originalvehprops.model)
				cb(false)
			end
		else
			cb(false)
