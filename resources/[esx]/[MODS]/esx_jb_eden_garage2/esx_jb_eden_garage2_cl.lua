...
function OpenMenuGarage(garage, KindOfVehicle)
	...
	local elements = {
		{label = "Trả xe ("..Config.Price.."$)", value = 'return_vehicle'},
	}
		...
		if not table.empty(vehicles) then
			for _,v in pairs(vehicles) do
				...
				if(v.fourrieremecano)then
					labelvehicle = vehicleName..': Gara'
				elseif (v.state)  then
					labelvehicle = vehicleName..': Trở lại'
				else
					labelvehicle = vehicleName..': Lối ra'
				end	
				table.insert(elements, {label =labelvehicle , value = v})
				
			end
		else
			table.insert(elements, {label ="Khong co xe trong gara" , value = nil})
		end
		ESX.UI.Menu.Open(
				...
				function(data2, menu2)
					if data2.current.value == "get_vehicle_out" then
                        if (data.current.value.fourrieremecano) then
                            TriggerEvent('esx:showNotification', 'Xe cua ban dang o khu lay xe')
                        elseif (data.current.value.state) then
                            menu.close()
                            menu2.close()
                            SpawnVehicle(data.current.value.vehicle, garage, KindOfVehicle)
                        else
                            TriggerEvent('esx:showNotification', 'Xe cua ban da het')
                        end
					elseif data2.current.value == "rename_vehicle" then
						AddTextEntry('FMMC_KEY_TIP8', "Tên của chiếc xe mong muốn")
...
function StockVehicleMenu(KindOfVehicle)
	...
	if IsPedInAnyVehicle(playerPed,  false) then
		...
		if GetPedInVehicleSeat(vehicle, -1) == playerPed then
			...
			if GotTrailer then
				...
				ESX.TriggerServerCallback('eden_garage:stockv',function(valid)
					if(valid) then
						...
						TriggerEvent('esx:showNotification', 'Xe cua ban dang o trong nha de xe')
					else
						TriggerEvent('esx:showNotification', 'Ban khong the luu tru chiec xe nay')
					end
			...
			TriggerEvent('esx:showNotification', 'Ban khong phai la nguoi dieu khien phuong tien')
		end
	else
		TriggerEvent('esx:showNotification', 'Khong co xe de vao')
	end
end
			...
			if GotTrailer then
				...
				ESX.TriggerServerCallback('eden_garage:stockvmecano',function(valid)
					if(valid) then
						...
						TriggerEvent('esx:showNotification', 'Xe da tro lai gara')
					else
						TriggerEvent('esx:showNotification', 'Ban khong the luu tru xe nay o vung cao')
					end
				end,trailerProps)
			else
				local vehicleProps  = ESX.Game.GetVehicleProperties(vehicle)
				ESX.TriggerServerCallback('eden_garage:stockvmecano',function(valid)
					if(valid) then
						...
						TriggerEvent('esx:showNotification', 'Xe da tro lai trong gara')
					else
						TriggerEvent('esx:showNotification', 'Ban khong the luu tru chiec xe nay o vung cao')
					end
				end,vehicleProps)
			end
		else
	...
	ESX.TriggerServerCallback('eden_garage:getOutVehicles', function(vehicles)
		...
		if not table.empty(vehicles) then
			for _,v in pairs(vehicles) do
				...
				if v.fourrieremecano then
					labelvehicle = vehicleName..': Gara ben ngoai'
			...
			table.insert(elements, {label ="Khong co xe de di ra ngoai" , value = nil})
		end
		...
		function(data, menu)
			if data.current.value == 'fourrieremecano' then
				ESX.ShowNotification("Lien he voi canh sat hoac mecano de tim hieu lam the nao de lay lai xe cua ban.")
				...
				if not iscaronearth then
					ESX.TriggerServerCallback('eden_garage:checkMoney', function(hasEnoughMoney)
						if hasEnoughMoney then
							...
							ESX.ShowNotification('Ban khong co du tien')						
						end
					end)
				else
					ESX.ShowNotification("Ban khong the lay chiec xe nay ra.")
				end				
			end
		end,
		function(data, menu)
