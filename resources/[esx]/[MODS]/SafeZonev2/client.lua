...
Citizen.CreateThread(function()
	...
	for i = 1, #zones, 1 do
		...
		SetBlipSprite(szBlip, 2) -- Change the blip itself: https://marekkraus.sk/gtav/blips/list.html
	...
	while true do
		...
		if dist <= 50.0 then  ------------------------------------------------------------------------------ Here you can change the RADIUS of the Safe Zone. Remember, whatever you put here will DOUBLE because 
			if not notifIn then																			  -- it is a sphere. So 50 will actually result in a diameter of 100. I assume it is meters. No clue to be honest.
				...
				TriggerEvent("pNotify:SendNotification",{
					text = "<b style='color:#1E90FF'>Bạn đang ở trong SafeZone</b>",
					type = "success",
					timeout = (3000),
					layout = "bottomcenter",
					queue = "global"
			...
			if not notifOut then
				...
				TriggerEvent("pNotify:SendNotification",{
					text = "<b style='color:#1E90FF'>Bạn đang ở ngoài SafeZone</b>",
					type = "error",
					timeout = (3000),
					layout = "bottomcenter",
					queue = "global"
      ...
      DisableControlAction(0,143,true) -- disable melee
			if IsDisabledControlJustPressed(2, 37) then --if Tab is pressed, send error message
				...
				TriggerEvent("pNotify:SendNotification",{
					text = "<b style='color:#1E90FF'>Bạn không thể sử dụng vũ khí trong Vùng an toàn</b>",
					type = "error",
					timeout = (3000),
					layout = "bottomcenter",
					queue = "global"
			...
			if IsDisabledControlJustPressed(0, 106) then --if LeftClick is pressed, send error message
				...
				TriggerEvent("pNotify:SendNotification",{
					text = "<b style='color:#1E90FF'>Bạn không thể làm điều đó trong Vùng an toàn</b>",
					type = "error",
					timeout = (3000),
					layout = "bottomcenter",
					queue = "global"
